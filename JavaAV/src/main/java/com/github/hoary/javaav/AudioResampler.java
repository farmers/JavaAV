...
public class AudioResampler {
    ...
    /**
     * Initializes the {@code AudioResampler} with specified input and output
     * audio formats. Invocations of {@link #resample(AudioFrame)} will return
     * audio frames with specified output audio format. The audio format of
     * provided audio frames must be equal to the input audio format specified
     * in this method.
     * 
     ...
     */
    public void open(AudioFormat srcFormat, AudioFormat dstFormat, int frameSamples)
        ...
        if (srcFormat.getChannelLayout() != null) {
            long srcChannelLayout = srcFormat.getChannelLayout().value();
            av_opt_set_int(convertContext, "icl", srcChannelLayout, 0);
            av_opt_set_int(convertContext, "ich",
                    av_get_channel_layout_nb_channels(srcChannelLayout),
                    0);
        }
        else {
            av_opt_set_int(convertContext, "ich", srcFormat.getChannels(),
                    0);
        }
        ...
        av_opt_set_int(convertContext, "och", av_get_channel_layout_nb_channels(dstChannelLayout),
                0);
    ...
    /**
     * Resample audio samples within the provided audio frame. The audio format
     * of the input audio frame must be equal to the audio format previously
     * specified with {@link #open(AudioFormat, AudioFormat, int)}. The
     * resampled samples are buffered, if they do not fit completely in one
     * output frame. Depending on output audio format more than one frame may be
     * returned.
     * 
    ...
    public AudioFrame[] resample(AudioFrame frame) {
        ...
        int destSamples = Math.max(frameSamples,
                (int) av_rescale_rnd(frameSamples, outputRate, inputRate, AV_ROUND_DOWN));
        ...
        int resampled = swr_convert(convertContext, tempFrame.getData(), outSamples, inPointer,
                inSamples);

        // re-sampled plane size
        int limit = (resampled * outputChannels * av_get_bytes_per_sample(outputFormat)) / planes;

        // write samples into buffer in case the current frame cannot be fully
        // filled
        ...
        int bufferSize = (destSamples * outputChannels * av_get_bytes_per_sample(outputFormat))
                / planes;
